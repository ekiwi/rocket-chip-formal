...
class PasoTLMonitor(argEdge: TLEdge, monitorDir: MonitorDirection = MonitorDirection.Monitor) extends Module {
  ...
  def legalizeFormatA(bundle: TLBundleA, edge: TLEdge): Unit = {
    ...
    when(bundle.opcode === TLMessages.AcquireBlock) {
      ...
      monAssert(
        edge.master.supportsProbe(edge.source(bundle), bundle.size) && edge.slave
          .emitsProbeSafe(edge.address(bundle), bundle.size),
  ...
  def legalizeFormatB(bundle: TLBundleB, edge: TLEdge): Unit = {
    ...
    when(bundle.opcode === TLMessages.PutFullData) {
      monAssert(
        edge.master.supportsPutFull(edge.source(bundle), bundle.size) && edge.slave
          .emitsPutFullSafe(edge.address(bundle), bundle.size),
    ...
    when(bundle.opcode === TLMessages.PutPartialData) {
      monAssert(
        edge.master.supportsPutPartial(edge.source(bundle), bundle.size) && edge.slave
          .emitsPutPartialSafe(edge.address(bundle), bundle.size),
    ...
    when(bundle.opcode === TLMessages.ArithmeticData) {
      monAssert(
        edge.master.supportsArithmetic(edge.source(bundle), bundle.size) && edge.slave
          .emitsArithmeticSafe(edge.address(bundle), bundle.size),
    ...
    when(bundle.opcode === TLMessages.LogicalData) {
      monAssert(
        edge.master.supportsLogical(edge.source(bundle), bundle.size) && edge.slave
          .emitsLogicalSafe(edge.address(bundle), bundle.size),
  ...
  def legalizeFormatC(bundle: TLBundleC, edge: TLEdge): Unit = {
    ...
    when(bundle.opcode === TLMessages.Release) {
      monAssert(
        edge.master.emitsAcquireB(edge.source(bundle), bundle.size) && edge.slave
          .supportsAcquireBSafe(edge.address(bundle), bundle.size),
  ...
  def legalizeADSource(bundle: TLBundle, edge: TLEdge): Unit = {
    ...
    when(bundle.d.valid && d_first && edge.isResponse(bundle.d.bits) && !d_release_ack) {
      val same_cycle_resp =
        bundle.a.valid && a_first && edge.isRequest(bundle.a.bits) && (bundle.a.bits.source === bundle.d.bits.source)
      assume(
  ...
  def legalizeCDSource(bundle: TLBundle, edge: TLEdge): Unit = {
    ...
    when(bundle.d.valid && d_first && edge.isResponse(bundle.d.bits) && d_release_ack) {
      val same_cycle_resp =
        bundle.c.valid && c_first && edge.isRequest(bundle.c.bits) && (bundle.c.bits.source === bundle.d.bits.source)
      assume(
        ((inflight)(bundle.d.bits.source)) || same_cycle_resp,
        "'D' channel acknowledged for nothing inflight" + extra
      )
    ...
    when(
      bundle.d.valid && d_first && c_first && bundle.c.valid && (bundle.c.bits.source === bundle.d.bits.source) && d_release_ack
    ) {
    ...
    if (edge.manager.minLatency > 0) {
      when(c_set_wo_ready.orR) {
        assume(
          ...
          s"'C' and 'D' concurrent, despite minlatency ${edge.manager.minLatency}" + extra
        )
      }
    }
